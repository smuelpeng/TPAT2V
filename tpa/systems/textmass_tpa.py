...
from tpa.models.loss import CLIPLoss
from tpa.utils.metrics import t2v_metrics, v2t_metrics
from tpa.utils.config import gen_log
...
class TextMassTPA(BaseSystem):
    ...
    def configure(self) -> None:
        ...
        self.window_metric = defaultdict(lambda: deque(maxlen=self.cfg.eval_window_size))
    ...
    def training_step(self, batch, batch_idx):
        ...
        output = sim_matrix_training(text_embeds_stochastic, video_embeds_pooled, self.cfg.pooling_type)
        ...
        output_support = sim_matrix_training(text_support, video_embeds_pooled, self.cfg.pooling_type)
        ...
        self.log( "train/loss_ori", loss)
        self.log( "train/loss_support", loss_support)
        self.log( "train/loss_tot", loss_all)
    ...
    def on_validation_end(self):
        ...
        text_embeds_stochastic_allpairs = torch.zeros(size=(vid_embeds.shape[0], text_embeds.shape[0], text_embeds.shape[1])).to(text_embeds.device)
        # @WJM: the principle is to use the query video to process text
        # sequential process to save memory:
        for (idx_vid, single_vid), single_vid_embed_pooled in tqdm(zip(enumerate(vid_embeds),vid_embeds_pooled), desc="Processing val videos"):
        ...
        gen_log(model_path=self.get_save_dir(), log_name='log_trntst', msg=msg)
        ...
        text_embeds_per_video_id, vid_embeds_pooled_per_video_id = generate_embeds_per_video_id_stochastic(text_embeds_stochastic_allpairs,
                vid_embeds_pooled, all_vid_ids, self.cfg.pooling_type)
        ...
        if self.cfg.save_memory_mode:
            ...
            gen_log(model_path=self.get_save_dir(), log_name='log_trntst', msg='Use sim_matrix_inference_stochastic_light()')
            sims = sim_matrix_inference_stochastic_light_allops(text_embeds_per_video_id, vid_embeds_pooled_per_video_id, self.cfg.pooling_type, self.cfg.batch_size_split, self.cfg)
            end_sims = time.time()
            gen_log(model_path=self.get_save_dir(), log_name='log_trntst', msg=f'batch size split = {self.cfg.batch_size_split}, sims compute time={end_sims-start_sims}')
        else:
            sims = sim_matrix_inference_stochastic(text_embeds_per_video_id, vid_embeds_pooled_per_video_id, self.cfg.pooling_type)

        total_val_loss = total_val_loss / len(self.validation_step_outputs)
